...
public class StringKit {
	...
	public static String substringBit(String str, int length, String suffix) {
		...
		try {
			byte[] bytes = str.getBytes(Constants.ENCODING_UNICODE);
			int n = 0; // 表示当前的字节数
			...
			return new String(bytes, 0, subLen, Constants.ENCODING_UNICODE) + suffix;
		} catch (Exception e) {
			e.printStackTrace();
			return "";
		}
	...
	public static String substringHtmlBit(String str, int length, String suffix) {
		...
		try {
			...
			if (subLen == bytes.length) {
				...
				result = new String(bytes, 0, subLen, Constants.ENCODING_UNICODE) + suffix;
			}
			String htmlStr = "";
