...
import com.lpmas.framework.util.EncodeKit;
...
public class MailKit {
	...
	public static boolean sendMail(SendMailBean bean) {
		...
		try {
			...
			mimeMessage.setRecipients(Message.RecipientType.TO, getAddresses(bean.getToUserList()));
			mimeMessage.setRecipients(Message.RecipientType.CC, getAddresses(bean.getCcUserList()));
			mimeMessage.setRecipients(Message.RecipientType.BCC, getAddresses(bean.getBccUserList()));

			// 处理附件
			if (bean.getAttachmentList() != null && bean.getAttachmentList().size() > 0) {
				for (String fileName : bean.getAttachmentList()) {
					MimeBodyPart attachmentMbp = getAttachment(fileName);
	 ...
	 */
	private static InternetAddress[] getAddresses(List<String> list) throws MessagingException {
	 ...
	 */
	private static MimeBodyPart getAttachment(String fileName) throws MessagingException, UnsupportedEncodingException {
		MimeBodyPart mimeBodyPart = new MimeBodyPart();
