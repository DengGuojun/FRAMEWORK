...
public class HttpResponseKit {
	 ...
	 *            关闭窗口
	 */
	public static void alertMessage(HttpServletResponse response, String message, int action) {
		try {
	 ...
	 * @param redirectUrl
	 */
	public static void alertMessage(HttpServletResponse response, String message, String redirectUrl) {
		try {
			setResponseHeader(response);
			PrintWriter out = response.getWriter();
			out.println("<script>");
	...
	public static void printMessage(HttpServletResponse response, String message) {
		try {
			printMessage(response, message, CT_HTML);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 */
	public static void printMessage(HttpServletResponse response, String message, String contentType) {
		try {
	...
	public static void setAttachmentFile(HttpServletRequest request, HttpServletResponse response, String fileName)
		...
		if (browserInfo[0].equals(BrowserKit.FIREFOX)) {
			response.setHeader("Content-disposition",
					"attachment;filename=\"" + EncodeKit.toSystemCode(fileName) + "\"");
		} else {
			response.setHeader("Content-disposition",
					"attachment;filename=\"" + URLEncoder.encode(fileName, Constants.ENCODING_UNICODE) + "\"");
		}
	}
}
